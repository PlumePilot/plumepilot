(() => {
  ...
  async function interactiveHtml() {
    ...
    function saveNote(question,field,value){const previous=notes.get(question.noteKey)||{explanation:'',observations:''};const next={...previous,[field]:value};if(!next.explanation&&!next.observations)notes.delete(question.noteKey);else notes.set(question.noteKey,next)}
    function noteEditor(question){const saved=notes.get(question.noteKey)||{explanation:'',observations:''};const details=document.createElement('details');details.className='personal-notes';details.open=Boolean(saved.explanation||saved.observations);const summary=document.createElement('summary');summary.textContent='Spiegazione e osservazioni';details.appendChild(summary);let activeField=null;for(const [field,labelText] of [['explanation','Spiegazione'],['observations','Osservazioni']]){const label=document.createElement('label');label.className='note-field';label.append(labelText);const textarea=document.createElement('textarea');textarea.value=saved[field]||'';textarea.placeholder=field==='explanation'?'Scrivi il procedimento o il motivo della risposta…':'Aggiungi dubbi, collegamenti o promemoria…';textarea.addEventListener('focus',()=>{activeField=textarea});textarea.addEventListener('input',()=>saveNote(question,field,textarea.value));label.appendChild(textarea);details.appendChild(label);if(!activeField)activeField=textarea}const bar=document.createElement('div');bar.className='symbol-bar';bar.setAttribute('aria-label','Simboli matematici');for(const symbol of symbols){const button=document.createElement('button');button.type='button';button.textContent=symbol;button.title='Inserisci '+symbol;button.addEventListener('mousedown',event=>event.preventDefault());button.addEventListener('click',()=>{const field=activeField;if(!field)return;const start=field.selectionStart;const end=field.selectionEnd;field.setRangeText(symbol,start,end,'end');field.dispatchEvent(new Event('input',{bubbles:true}));field.focus()});bar.appendChild(button)}details.appendChild(bar);return details}
    ...
    byId('check').onclick=()=>reveal(true);byId('reset').onclick=()=>{results.delete(current);render()};byId('downloadNotes').onclick=downloadAnnotated;byId('showSolutions').onchange=()=>byId('showSolutions').checked?reveal(false):render();byId('shuffleQuestions').onchange=render;byId('shuffleAnswers').onchange=render;render();
    </script></body></html>`;
  }
