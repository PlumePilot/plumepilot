...
The HTML reuses the offline test-collection layout and exact approved palette: a chapter sidebar, one page per chapter and video cards containing the original formatted notes. Each source note has a collapsible integration field with the same mathematical-symbol palette and saved-note colors used by the test collection. The PDF now follows the same visual hierarchy and palette, with a cover, clickable chapter index, module/chapter/video headings and hierarchical PDF bookmarks. Download/reopen and unsaved-change prompts require browser smoke testing. Raw tracking_time is preserved in HTML metadata, not displayed with an unverified unit. Remote note images become explicit placeholders. Unsupported PDF glyphs cause a visible error suggesting the lossless HTML format rather than silently replacing content.
...
Add one course-notes collection action in both activity interfaces, with start/cancel/status behavior and no duplicated state ownership. The popup action uses the same expandable description control as the other actions, but help panels open without height animation so browser-popup resizing cannot produce intermediate jumps. Both interfaces use the normalized `action-course-notes.png` Gaming sprite and the established compact-button structure; the floating action spans the same full grid width as test collection and course materials. Keep cancellation reachable irrespective of selected tab. Integrate Standard/Gaming, Light/Dark/System and S/M/L layouts.
